...
hold ('all')
for i = 1:ntrials
hs = plot(EEG.times,data1(1:end-1,i),'color',lgrey,'linew',0.1);
set(get(get(hs,'Annotation'),'LegendInformation'),'IconDisplayStyle','off');

ht = plot(EEG.times,data2(1:end-1,i)-5,'color',lgrey,'linew',0.1);
set(get(get(ht,'Annotation'),'LegendInformation'),'IconDisplayStyle','off');

end
plot(EEG.times,squeeze(mean(EEG.data(1,:,:),3)),'b','linew',3 )
plot(EEG.times,squeeze(mean(EEG.data(2,:,:),3))-5,'g','linew',3 )
xlim([-400 1000])
