...
% subplot(312)
% plot(EEG.times,angles1(1:end-1,1),'k')
% hold on
% plot(EEG.times,angles2(1:end-1,1)-10,'k')
% ylabel('Phase angle')
...
subplot(221)
hold ('all')
hs = plot(EEG.times,signal1(1:end-1,:),'color',lgrey,'linew',0.1);
ht = plot(EEG.times,signal2(1:end-1,:)-5,'color',lgrey,'linew',0.1);
plot(EEG.times,squeeze(mean(signal1(1:end-1,:),2)),'b','linew',3)
plot(EEG.times,squeeze(mean(signal2(1:end-1,:),2))-5,'g','linew',3)
ylabel('Amplitude')
title('Amplitude over time')
% legend({'Single trial data'})
