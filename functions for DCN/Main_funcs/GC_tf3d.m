...
function tf_granger = GC_tf3d(data1, data2, freqs, srate, order,twin,times2saveidx)
...
times2save = -1500:200:1500;
twin_pnts = round(twin/(1000/srate));
tf_granger=zeros(2,length(freqs),length(times2save));
...
for ti=1:length(times2save) % loop over time points
    
    % figure out time points, considering edges
    tidx = times2saveidx(ti) - ceil(twin_pnts/2) : times2saveidx(ti) + ceil(twin_pnts/2);
